...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const requestUrl = `${baseUrl}/refresh/${id}`;
    console.log('Sending analysis refresh request to:', requestUrl);
    ...
    const response = await fetch(`${baseUrl}/refresh/${id}`, {
      ...
      body: req.method !== 'GET' ? JSON.stringify(req.body) : undefined, // Don't include body for GET requests
    });

    // Check if response is JSON
    const contentType = response.headers.get('content-type');
    if (!contentType || !contentType.includes('application/json')) {
      console.error(`Analysis refresh endpoint returned non-JSON response (${contentType})`);
    ...
    console.log(`Analysis refresh response for ID ${id}:`, {
